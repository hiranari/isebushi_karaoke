...
- **改善内容**:
  ...
  - プログレッシブディスクロージャー（総合スコア → 詳細分析 → 改善提案）
  - 歌唱後の詳細分析結果表示
  - 改善ポイントの具体的提案機能（動的生成）
- **技術的実装**: 
  - UI/UXとビジネスロジックの完全分離
  - Provider状態管理による画面状態制御
  - 独立したサービスクラス（スコア計算・分析・改善提案）
  - 統一されたSongResultデータモデル
...
## Phase 3: 設計指針・保守性ガイドライン

### アーキテクチャ原則
- **責務分離の徹底**: UI層はデータ表示のみ、ビジネスロジックはservices層に集約
- **単一責務の原則**: 1クラス1責務、100行ルールの遵守
- **独立性の確保**: スコア計算・分析・改善提案はそれぞれ独立したサービス
- **状態管理の分離**: Provider/ChangeNotifierによる画面状態の適切な管理

### データモデル設計
- **SongResult**: 歌唱結果の一元管理モデル
- **ComprehensiveScore**: 多角的評価指標（音程70%・安定性20%・タイミング10%）
- **DetailedAnalysis**: 詳細分析結果とグラフデータ
- **ImprovementSuggestion**: 動的改善提案生成

### UI/UXパターン
- **プログレッシブディスクロージャー**: 総合スコア → 詳細分析 → 改善提案
- **段階的情報開示**: ユーザーの認知負荷を最小化
- **一貫したナビゲーション**: 各段階間の明確な遷移
